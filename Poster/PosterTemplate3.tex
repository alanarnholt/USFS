...
\item Another bullet here
\item Yet a fifth bullet here
\item A sixth bullet if you want
\end{itemize}
\vspace{0ex}
\end{block}
\vfill
...
\hlstd{x} \hlkwb{<-} \hlkwd{rnorm}\hlstd{(}\hlnum{100}\hlstd{,} \hlnum{0}\hlstd{,} \hlnum{1}\hlstd{)}
\hlstd{results} \hlkwb{<-} \hlkwd{numeric}\hlstd{(N)}
\hlkwa{for}\hlstd{(i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{N)\{}
  \hlstd{bs} \hlkwb{<-} \hlkwd{sample}\hlstd{(x,} \hlkwc{size} \hlstd{=} \hlnum{100}\hlstd{,} \hlkwc{replace} \hlstd{=} \hlnum{TRUE}\hlstd{)}
  \hlstd{results[i]} \hlkwb{<-} \hlkwd{mean}\hlstd{(bs)}
\hlstd{\}}
...
{\centering \includegraphics[width=\maxwidth]{figure/HIST3b-1} 
