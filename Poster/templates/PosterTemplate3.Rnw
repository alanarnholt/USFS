...
library(xtable)
library(ggmap)
housedata <- read.csv("../PData/housedata.csv", 
...
housedataT <- read.csv("../PData/housedataTEST.csv",
                      colClasses = c(id = "character", date = "character", 
                                     yr_built = "character", zipcode = "factor", grade = "factor"))
...
<<label = "HIST3b", echo = FALSE, fig.height = 6.5>>=
hist(rnorm(1000), col = "lightblue")
@
\vspace{0ex}
\vfill
\end{block}
...
\vspace{0ex}
<<label = "HIST2", echo = FALSE, fig.height = 7.5>>=
...
<<label = "HIST3", echo = FALSE, fig.height = 7.5>>=
