...
# FROM 1899-1956
hair1958 <- read.xlsx("hair1958.xlsx",1,header = FALSE)
...
ulrich4 <- read.xlsx("ulrich4.xlsx",1,header=FALSE)
...
ulrich5 <- read.xlsx("ulrich5.xlsx", 1, header = FALSE)
...
########################
###### ULRICH TABLE 6
ulrich6 <- read.xlsx("UlrichTable6.xlsx", 1, header=FALSE,
...
#####
###HOWARD 5A
howard5 <- read.xlsx("howard5.xlsx", 1, header=FALSE)
...
howard6 <- read.xlsx("howard6a.xlsx", 1, header=FALSE)
...
howard7 <- read.xlsx("howard7a.xlsx", 1, header=FALSE)
...
howard28 <- read.xlsx("howard28.xlsx",1,header=FALSE)
...
#########
####HOWARD 46
howard46 <- read.xlsx("howard46.xlsx",1,header=FALSE)
...
######
###Howard 47
howard47 <- read.xlsx("howard47.xlsx",1,header=FALSE)
rownames(howard47) <- 1965:2020
h47 <- function(y,c){
  return(howard47[y-1964,c])
}
##HOWard 49
howard49 <- read.xlsx("howard49.xlsx",1,header=FALSE)
rownames(howard49) <- 1965:2020
h49 <- function(y,c){
  return(howard49[y-1964,c])
}
####HOWARD 53
howard53 <- read.xlsx("howard53.xlsx",1,header=FALSE)
rownames(howard53) <- 1965:2020
h53 <- function(y,c){
    return(howard53[y-1964,c])
}
#####HOWARD 55
howard55 <- read.xlsx("howard55.xlsx",1,header=FALSE)
rownames(howard55) <- 1965:2020
h55 <- function(y,c){
    return(howard55[y-1964,c])
}
######HOWARD 56
howard56 <- read.xlsx("howard56.xlsx",1,header=FALSE)
rownames(howard55) <- 1965:2020
h56 <- function(y,c){
  return(howard56[y-1964,c])
}
####INce table1
ince1 <- read.xlsx("ince1.xlsx",1,header=FALSE)
rownames(ince1) <- 1900:2050
inc1 <- function(y,c){
    return(ince1[y-1899,c])
}
######fraction of sawnwood used in various end uses 1900-2050
fracsawnwood <- read.xlsx("fracsawnwood.xlsx", 1, header = FALSE)
...
fracstrpanels <- read.xlsx("fracstrpanels.xlsx",1, header = FALSE)
...
halfLives <- read.xlsx("halfLives.xlsx", 1,header=FALSE)
rownames(halfLives) <- 1900:2050
HL <- function(y,eu){
  return(halfLives[y-1899,eu])
}
######LOSS when placed IU based on EU and yr
lossIU <- read.xlsx("lossWhenPlacedIU.xlsx",1,header=FALSE)
...
sFcarbon <- read.xlsx("sfHomeCheck.xlsx",1,header=FALSE)
rownames(sFcarbon) <- 1900:2050
sfcarb <- function(y,c){
  return(sFcarbon[y-1899,c])
}
#######TOTAL CARBON CHECK 1900:2050
totCarbon <- read.xlsx("checkTotalCarbon.xlsx",1,header=FALSE)
...
swPcorrect <- read.xlsx("swPcorrect.xlsx",1,header=FALSE)
...
lumberpre1900 <- read.xlsx("lumberpre1900.xlsx",1,header=FALSE)
...
##################
########PAPER
IncePaper <- read.xlsx("Ince_Paper.xlsx",1,header=FALSE)
rownames(IncePaper) <- 1899:2050
getIncePap <- function(y,c){
  return(IncePaper[y-1898,c])
}
apiFiberpulp <- read.xlsx("api1975Fiberpulp.xlsx",1,header=FALSE)
